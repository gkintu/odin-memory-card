      ...
      try {
        ...
        const detailedPromises = data.results.map(async (pokemon) => {
          ...
          if (!detailResponse.ok) {
            // Check if detail fetch was successful
          ...
          return {
            // Return an object with the data we need
  ...
  const handleCardClick = (pokemonId) => {
    ...
    if (clickedPokemonIds.includes(pokemonId)) {
      ...
      if (newScore === numberOfPokemon) {
        ...
        console.log("You Win!");
        alert(
          `You Win! You clicked all ${numberOfPokemon} Pokemon without repeating! Play again?`
        ); // Win message
        // Reset for the next round
        setCurrentScore(0); // Reset current score
        setClickedPokemonIds([]); // Reset clicked IDs
        // Best score is already updated above
      }
    }
